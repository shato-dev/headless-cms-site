...
| ランダムおすすめ(「今日の一冊」) | ボタン1つで100作品からランダムに1冊選び、詳細ページへ | クライアント JS で `id` 配列から抽選 → `location.href` 遷移。サーバー不要 | ✅ 完了(2026-09-23) |
| 診断式おすすめ | 気分・長さ・ジャンルなど数問に答えると1冊を提案 | まずは `genreTags` ベースの簡易スコアリングから開始(全文検索や本格的なレコメンドは将来 M5/M6 と絡めて発展させる余地あり) | ✅ 完了(2026-09-23) |
| 関連作品(提案) | 詳細ページ下部に「同じ著者の他の作品」「同じジャンルの作品」 | `author`/`genreTags` での単純フィルタ、追加データ不要 | ✅ 完了(2026-09-23) |
...
**ランダムおすすめの `events` テーブル連携について(2026-09-23 決定)**: 実装時に
`random_pick` イベントを M5 の `events` テーブルへ送るかを検討したが、**送らない**ことにした。
M5 で「書き込み API の公開はスコープ外(認証・レート制限が要る)」と決定済みで、それを覆すほどの
理由が今回は無いため。クライアント完結(`search.json` を fetch してランダム選出 → 遷移)のみ。
events 送信が必要になったら、そのとき改めて書き込み API を設計する。

**診断式おすすめの実装メモ(2026-09-23)**: `/quiz` ページ。2問(気分・想定時間)の選択式で、
各選択肢が `genreTags` に重み付きで対応。合計スコアが最も高い作品群からランダムに1件選出
(未回答時は全体からランダム = ランダムおすすめと同じ挙動にフォールバック)。データはこのページ
専用なのでビルド時に `getWorks()` で取得し埋め込み(`RandomPick`/`Search` のような `/search.json`
fetch ではなく、ページ単体でしか使わないため往復が不要)。**ハマりどころ**: Astro は `<script>`
タグ内の `{式}` を評価しない(そのまま `{...}` という文字列として出力される)。データをスクリプトに
埋め込むには `define:vars`(ただし IIFE でラップされ JSON 単体にはならない)か、`set:html` で明示的に
生テキストとして注入する必要がある。`JSON.stringify()` に加えて `<` を `<` に置換
(`</script>` 注入対策の保険。Astro 自身の `define:vars` シリアライズも同じ変換をしていることを
ローカルビルドで確認済み)。
