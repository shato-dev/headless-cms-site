...
- [ ] **最初に「Postgres の用途」を決める**(何のデータを入れ、誰が何のために使うか)。候補は PLAN.md の
      M5 節「用途の候補」(A 拡張メタデータ / B 閲覧・操作ログ / C 既読・お気に入り / D 検索クエリログ)。
      microCMS(コンテンツの正本)との役割分担も決める
- [ ] `docker compose up -d` でローカル Postgres を起動(Docker Desktop の起動確認が先)
- [ ] スキーマ設計(可変・半構造データは JSONB、列に分けるものとの線引き)
- [ ] 本番 DB の選定: 用途が決まってから、Supabase / Neon の無料プランについて、カード要否・無料枠・
      休止条件・Workers からの接続経路を整理して選ぶ
